...
export type FlowMapSelectorProps = Props;
...
export const getFetchedFlows = (state: FlowMapState, props: Props) =>
  props.flows;
export const getFetchedLocations = (state: FlowMapState, props: Props) =>
  props.locations;
...
export const getZoom = (state: FlowMapState, props: Props) =>
  state.viewport.zoom;
export const getViewport = (state: FlowMapState, props: Props) =>
  state.viewport;
export const getSelectedTimeRange = (state: FlowMapState, props: Props) =>
  state.filterState.selectedTimeRange;

export const getInvalidLocationIds: Selector<string[] | undefined> =
  createSelector(getFetchedLocations, (locations) => {
...
export const getSelectedLocationsSet: Selector<Set<string> | undefined> =
  createSelector(getSelectedLocations, (ids) =>
    ids && ids.length > 0 ? new Set(ids) : undefined,
  );

export const getSortedFlowsForKnownLocations: Selector<Flow[] | undefined> =
  createSelector(getFetchedFlows, getLocationIds, (flows, ids) => {
...
export const getTimeGranularityKey: Selector<TimeGranularityKey | undefined> =
  createSelector(
    getSortedFlowsForKnownLocations,
    getActualTimeExtent,
    (flows, timeExtent) => {
...
export const getLocationsHavingFlows: Selector<Location[] | undefined> =
  createSelector(
    getSortedFlowsForKnownLocations,
    getLocations,
    (flows, locations) => {
...
export const getLocationsById: Selector<Map<string, Location> | undefined> =
  createSelector(getLocationsHavingFlows, (locations) => {
    if (!locations) return undefined;
...
export const getClusterIndex: Selector<ClusterIndex | undefined> =
  createSelector(
...
export const getDarkMode: Selector<boolean> = (
  state: FlowMapState,
  props: Props,
) => state.settingsState.darkMode;

export const getFadeEnabled: Selector<boolean> = (
  state: FlowMapState,
  props: Props,
) => state.settingsState.fadeEnabled;
export const getFadeAmount: Selector<number> = (
  state: FlowMapState,
  props: Props,
) => state.settingsState.fadeAmount;

export const getAnimate: Selector<boolean> = (
  state: FlowMapState,
  props: Props,
) => state.settingsState.animationEnabled;

const _getFlowMapColors = createSelector(
  getDiffMode,
  getColorSchemeKey,
  getDarkMode,
  getFadeEnabled,
...
export const getFlowMapColorsRGBA = createSelector(
  _getFlowMapColors,
  (flowMapColors) => {
...
export const getUnknownLocations: Selector<Set<string> | undefined> =
  createSelector(
    getLocationIds,
    getFetchedFlows,
    getSortedFlowsForKnownLocations,
...
export const getSortedAggregatedFilteredFlows: Selector<Flow[] | undefined> =
  createSelector(
    getClusterIndex,
    getClusteringEnabled,
...
const _getFlowMagnitudeExtent: Selector<[number, number] | undefined> =
  createSelector(
...
export function getFlowMagnitudeExtent(state: FlowMapState, props: Props) {
  if (state.settingsState.adaptiveScalesEnabled) {
    const flows = getFlowsForFlowMapLayer(state, props);
    if (flows) {
      return extent(flows, getFlowMagnitude);
    } else {
      return undefined;
    }
  } else {
    return _getFlowMagnitudeExtent(state, props);
  }
}

export const getExpandedSelectedLocationsSet: Selector<
...
export const getTotalCountsByTime: Selector<CountByTime[] | undefined> =
  createSelector(
    getSortedFlowsForKnownLocations,
    getTimeGranularityKey,
    getTimeExtent,
...
const getViewportBoundingBox: Selector<[number, number, number, number]> =
  createSelector(
    getViewport,
...
const getLocationsForZoom: Selector<Location[] | ClusterNode[] | undefined> =
  createSelector(
    getClusteringEnabled,
    getLocationsHavingFlows,
    getClusterIndex,
...
export const getLocationTotals: Selector<
  Map<string, LocationTotals> | undefined
> = createSelector(
  getLocationsForZoom,
  getSortedAggregatedFilteredFlows,
  getSelectedLocationsSet,
...
const _getLocationIdsInViewport: Selector<Set<string> | undefined> =
  createSelector(
    getLocationsTree,
...
export const getTotalUnfilteredCount: Selector<number | undefined> =
  createSelector(getSortedFlowsForKnownLocations, (flows) => {
    if (!flows) return undefined;
    return flows.reduce((m, flow) => m + getFlowMagnitude(flow), 0);
  });

export const getTotalFilteredCount: Selector<number | undefined> =
  createSelector(
    getSortedAggregatedFilteredFlows,
    getSelectedLocationsSet,
    getLocationFilterMode,
...
const _getLocationTotalsExtent: Selector<[number, number] | undefined> =
...
export const getFlowsForFlowMapLayer: Selector<Flow[] | undefined> =
  createSelector(
    getSortedAggregatedFilteredFlows,
    getLocationIdsInViewport,
    getSelectedLocationsSet,
...
export const getLocationMaxAbsTotalGetter = createSelector(
  getLocationTotals,
  (locationTotals) => {
...
export const getFlowThicknessScale = (state: FlowMapState, props: Props) => {
  const magnitudeExtent = getFlowMagnitudeExtent(state, props);
...
export const getCircleSizeScale = (state: FlowMapState, props: Props) => {
  const maxLocationCircleSize = getMaxLocationCircleSize(state, props);
  const {locationTotalsEnabled} = state.settingsState;
  if (!locationTotalsEnabled) {
    return () => maxLocationCircleSize;
  }

  const locationTotalsExtent = getLocationTotalsExtent(state, props);
  ...
  return scaleSqrt()
    ...
    .domain([
      ...
      Math.max.apply(
        null,
        locationTotalsExtent.map((x) => Math.abs(x || 0)),
      ),
    ]);
};

export const getInCircleSizeGetter = createSelector(
  getCircleSizeScale,
  getLocationTotals,
...
export const getOutCircleSizeGetter = createSelector(
  getCircleSizeScale,
  getLocationTotals,
  (circleSizeScale, locationTotals) => {
    return (locationId: string) => {
...
const getSortedLocationsForZoom: Selector<
...
export const getLocationsForFlowMapLayerById: Selector<
  Map<string, Location | ClusterNode> | undefined
> = createSelector(getLocationsForFlowMapLayer, (locations) => {
  if (!locations) return undefined;
  return locations.reduce((m, d) => (m.set(getLocationId(d), d), m), new Map());
});
